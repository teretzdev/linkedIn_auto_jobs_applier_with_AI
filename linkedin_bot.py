...
class ConfigValidator:
    ...
    def validate_secrets(secrets_yaml_path: Path) -> tuple:
        ...
        mandatory_secrets = ['email', 'password', 'gemini_api_key'] # Updated mandatory secrets
        ...
        if not secrets['gemini_api_key']: # Updated validation for Gemini API key
            raise ConfigError(f"Gemini API key cannot be empty in secrets file {secrets_yaml_path}.")

        return secrets['email'], str(secrets['password']), secrets['gemini_api_key'] # Return Gemini API key
...
def create_and_run_bot(email: str, password: str, parameters: dict, gemini_api_key: str): # Pass Gemini API key
    try:
...
def main(resume: Path = None):
    try:
        ...
        email, password, gemini_api_key = ConfigValidator.validate_secrets(secrets_file) # Get Gemini API key from secrets
        parameters['uploads'] = FileManager.file_paths_to_dict(resume, plain_text_resume_file)
        parameters['outputFileDirectory'] = output_folder

        create_and_run_bot(email, password, parameters, gemini_api_key) # Pass Gemini API key to the bot

...
class ConfigValidator:
    ...
    def validate_secrets(secrets_yaml_path: Path) -> tuple:
        ...
        mandatory_secrets = ['email', 'password', 'google_api_key']
        ...
        return secrets['email'], str(secrets['password']), secrets['google_api_key']
...
def create_and_run_bot(email: str, password: str, parameters: dict, google_api_key: str):
    try:
        browser = init_browser()
        login_component = LinkedInAuthenticator(browser)
        apply_component = LinkedInJobManager(browser)
        gpt_answerer_component = GPTAnswerer(None, google_api_key)
...
def main(resume: Path = None):
    try:
        ...
        email, password, google_api_key = ConfigValidator.validate_secrets(secrets_file)
        parameters['uploads'] = FileManager.file_paths_to_dict(resume, plain_text_resume_file)
        parameters['outputFileDirectory'] = output_folder

        create_and_run_bot(email, password, parameters, google_api_key)

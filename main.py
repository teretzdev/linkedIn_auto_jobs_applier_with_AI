        ...
        if resume is None:
            ...
            if default_resume_path:
                ...
                if not resume.exists() or not resume.is_file():
                    logging.error(f"Default resume path is invalid or does not point to a file: {resume}")
                    printyellow("The default resume path specified in the configuration is invalid. Please check the logs for details.")
                    printyellow("Ensure the path exists and points to a valid file.")
                    return
